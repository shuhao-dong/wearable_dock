...
static int copy_file(int in, int out) {
    ...
    while ((r = read(in, buf, B)) > 0)
        if (write(out, buf, r) != r) { r = -1; break; }
    free(buf);
    return r<0 ? -1 : 0;
}
...
static int perform_dfu(const char *serial,const char *bin) {
    ...
    if(run_child(av1)) return fprintf(stderr,"DFU detach failed\n"),-1;
    sleep(2);
    char *av2[]={(char*)DFU_UTIL,"-a","1","-D",(char*)bin,NULL};
    if(run_child(av2)) return fprintf(stderr,"DFU download failed\n"),-1;
    mkdir(FW_ARCHIVE,0755);
    char arch[PATH_MAX];
    time_t t=time(NULL);
    struct tm tm; localtime_r(&t,&tm);
    strftime(arch,sizeof arch,FW_ARCHIVE"/%Y%m%d_%H%M%S.bin",&tm);
    if(rename(bin,arch)) { perror("rename firmware"); unlink(bin); }
    return 0;
}
...
static int is_fuse_mounted(const char *mp) {
    struct statfs s; return statfs(mp,&s)==0&&s.f_type==FUSE_SUPER_MAGIC;
}
...
static void handle_device(struct udev *udev) {
    ...
    if(is_fuse_mounted(MOUNT_POINT)) umount_mp(MOUNT_POINT);
...
int main(void){
    ...
    while(!quit_flag){
        ...
        if(!debouncing && act&&vid&&pid
           &&!strcmp(act,"add")
           &&!strcmp(vid,VENDOR_ID)&&!strcmp(pid,PRODUCT_ID))
        {
            puts("Wearable detected ? processing");
            handle_device(udev);
            puts("Waiting for removal?");
            strncpy(current_usb_path,spath,sizeof current_usb_path-1);
            debouncing=1; remove_seen_at_ms=0;
        }
        else if(debouncing && act && !strcmp(act,"remove")
                &&!strcmp(spath,current_usb_path))
        {
